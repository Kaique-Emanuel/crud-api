...
public class ProdutoController {
    ...
    public ResponseEntity<?> reativarProduto(@PathVariable Long id){
        Produto produto = service.reativarProduto(id);
    ...
    public ResponseEntity<?> deletarProduto(@PathVariable Long id){
        boolean excluido = service.excluirProduto(id);
        if (excluido) {
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(java.util.Map.of(
            "mensagem", "Este produto possui vendas registradas e não pode ser excluído. Ele foi desativado e não aparecerá mais na listagem, mas seu histórico de vendas foi preservado.",
            "acao", "desativado"
        ));
    }
...
@PutMapping("/{id}")
    ...
    public ResponseEntity<?> venderProduto(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        ...
        venda.setValorUnitario(produto.getValor() != null ? produto.getValor() : 0.0);
        venda.setValorTotal(venda.getValorUnitario() * quantidadeVendida);
        venda.setFormaPagamento(formaPagamento);
        venda.setDataHora(LocalDateTime.now());

        return ResponseEntity.ok(service.registrarVenda(produto, venda));
    }
